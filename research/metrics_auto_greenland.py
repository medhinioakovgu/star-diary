...
OUTPUT_CSV    = BASE / "data" / "greenland" / "automatic_metrics.csv"
...
# ── DATA LOADING ──────────────────────────────────────────────────────────────
def load_week_by_dirname(dirname: str) -> dict:
    """Load week JSON by trying <dirname>.json first, falling back to synthetic pattern."""
    path = WEEKS_DIR / f"{dirname}.json"
    if not path.exists():
        try:
            week_id = int(dirname.replace("week_", ""))
            path = WEEKS_DIR / f"week_{week_id:02d}.json"
        except ValueError:
            raise FileNotFoundError(f"No week file for dirname '{dirname}'")
    return json.loads(path.read_text(encoding="utf-8"))


def load_result_by_dirname(dirname: str, pipeline: str, gen_model: str) -> Optional[dict]:
    path = RESULTS_DIR / dirname / gen_model / f"{pipeline}.json"
    if not path.exists():
        return None
    return json.loads(path.read_text(encoding="utf-8"))
...
def main() -> None:
    ...
    parser.add_argument(
        "--weeks_filter", nargs="+", default=None,
        help="Filter result dirs by name substring, e.g. --weeks_filter greenland",
    )
    ...
    all_week_dirs = sorted(d for d in RESULTS_DIR.iterdir() if d.is_dir())
    targets: list[tuple[str, str]] = []  # (dirname, gen_model)
    for wd in all_week_dirs:
        if args.weeks_filter and not any(f in wd.name for f in args.weeks_filter):
            continue
        for model_dir in sorted(wd.iterdir()):
            if model_dir.is_dir():
                targets.append((wd.name, model_dir.name))
    ...
    for dirname, gen_model in targets:
        try:
            week = load_week_by_dirname(dirname)
        except FileNotFoundError:
            print(f"  Week file not found for {dirname}, skipping.")
            continue
        week_id = week["week_id"]
        reference = get_reference_text(week)

        for pipeline in PIPELINES:
            key = (dirname, pipeline, gen_model)
            if key in already_done:
                print(f"  {dirname} / {pipeline} / {gen_model} — already done, skipping.")
                continue

            result = load_result_by_dirname(dirname, pipeline, gen_model)
            if result is None:
