...
def main():
    ...
    for week_dir in sorted(results_root.iterdir()):
        if not week_dir.is_dir():
            continue
        # Look for matching week JSON: try both `<dirname>.json` and synthetic fallback
        week_path = weeks_root / f"{week_dir.name}.json"
        if not week_path.exists():
            # Fallback to synthetic naming (week_XX)
            try:
                week_id_str = week_dir.name.replace("week_", "")
                week_id = int(week_id_str)
                week_path = weeks_root / f"week_{week_id:02d}.json"
            except ValueError:
                print(f"Cannot resolve week file for {week_dir.name}, skipping.")
                continue
        if not week_path.exists():
